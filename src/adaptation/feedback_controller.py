...
class FeedbackController(Component):
    ...
    def _diagnose(self, context, features, confidence):
        ...
        disagreement = features.get("dense_bm25_agreement", 0.0)
        ...
        if query_type == "lexical" and strategy == "dense":
            return ("lexical_strategy_mismatch", "The query is lexical but dense retrieval is currently being used.")

        if query_type == "semantic" and strategy == "bm25":
            return ("semantic_strategy_mismatch", "The query is semantic but lexical retrieval is currently being used.")

        if query_type == "ambiguous" and strategy == "dense":
            return ("ambiguous_strategy_risk", "The query is ambiguous and dense retrieval may benefit from lexical support.")

        if query_type == "comparison" and strategy == "hybrid" and top_k < 10:
            return ("comparison_coverage_risk", "Comparison queries may require broader evidence coverage.")

        if confidence < 0.50:
            return ("weak_evidence", "Evidence confidence is below the medium-confidence range.")

        return ("uncertain_failure", "Evidence was rejected but no dominant failure pattern was detected.")
    ...
    def _diagnostic_action(self, context, diagnosis):
        ...
        if diagnosis in {"coverage_gap", "ranking_uncertainty", "comparison_coverage_risk"}:
    ...
    def _policy_action(self, context, query_type, current_strategy, confidence_bucket, current_top_k):
        ...
        action = result.get("selected_action", "keep")
        if action == "keep":
            return None
        ...
        candidate = result.get("candidates", {}).get(action, {})
    ...
    def _expected_improvement(self, diagnosis, action):
        ...
        if diagnosis == "lexical_strategy_mismatch":
            if action in {"switch_to_bm25", "switch_to_hybrid"}:
                return 0.10
            return 0.04

        if diagnosis == "semantic_strategy_mismatch":
            if action in {"switch_to_dense", "switch_to_hybrid"}:
                return 0.10
            return 0.04
    ...
    def run(self, context):
        ...
        policy_candidate = self._policy_action(
            context=context,
            query_type=query_type,
            current_strategy=current_strategy,
            confidence_bucket=confidence_bucket,
        ...
        if policy_candidate is not None:
            policy_action = policy_candidate["action"]
            policy_support = policy_candidate["support"]
            ...
            if policy_support >= 5:
                if diagnosis in {
                    ...
                    source = "policy_guided_diagnosis"
                    reason = (
                        "The frozen policy provided a supported action consistent "
                        "with the diagnosed retrieval failure."
                    )
        ...
        should_retry = (
            ...
            and
            expected_improvement >= self.minimum_improvement
        )
        ...
        if selected_action.startswith("switch_to_"):
            ...
            if target_strategy == current_strategy:
                ...
                reason = "The selected strategy matches the current strategy."
                source = "diagnostic_controller"
                target_strategy = None
            ...
            if target_top_k <= current_top_k:
                ...
                should_retry = False
                expected_improvement = 0.0
                reason = "The selected Top-K does not expand the current retrieval breadth."
                source = "diagnostic_controller"
                target_top_k = None
