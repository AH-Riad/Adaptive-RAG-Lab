...
class DenseRetriever(BaseRetriever):
    def __init__(self, embedding_model, vector_store, top_k: int = 3):
    ...
    def retrieve(self, query: str) -> RetrievalResult:
        ...
        if raw_results and "ids" in raw_results and len(raw_results["ids"]) > 0:
            ...
            documents = raw_results["documents"][0] if "documents" in raw_results else [""] * len(ids)
            metadatas = raw_results["metadatas"][0] if "metadatas" in raw_results else [{}] * len(ids)
