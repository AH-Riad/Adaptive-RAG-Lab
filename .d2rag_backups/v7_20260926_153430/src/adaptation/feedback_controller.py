...
class FeedbackController(Component):
    """Runtime controller for the frozen failure-conditioned D²RAG policy."""
    ...
    def _get_policy_record(
        ...
        record = policy_section.get(state)
        if record is not None:
            return record

        legacy_section = self.policy.get(section, {})
        legacy_state = self._legacy_policy_state(
            query_type=query_type,
            current_strategy=current_strategy,
            confidence_bucket=confidence_bucket,
            top_k=current_top_k,
        )
        return legacy_section.get(legacy_state)
    ...
    def _policy_candidate(self, record, current_top_k):
        ...
        average_gain = float(
            candidate.get(
                "average_gain",
                candidate.get("utility_gain", 0.0),
            )
        )
        average_utility = float(
            candidate.get("average_utility", 0.0)
        )
        ...
        return {
            ...
            "source": "failure_conditioned_policy_v6",
    ...
    def _learned_action(
        ...
        combined_record = self._get_policy_record(
            section="combined_policy",
            query_type=query_type,
            current_strategy=current_strategy,
            confidence_bucket=confidence_bucket,
            diagnosis=diagnosis,
        ...
        if diagnosis in self.STRATEGY_DIAGNOSES:
            ordered = [strategy_record, combined_record, topk_record]
        elif diagnosis in self.TOP_K_DIAGNOSES:
            ordered = [topk_record, combined_record, strategy_record]
        else:
            ordered = [combined_record, strategy_record, topk_record]

        candidates = []
        for record in ordered:
            candidate = self._policy_candidate(record, current_top_k)
    ...
    def _diagnostic_action(self, context, diagnosis):
        ...
        if (
            ...
            and strategy_action not in previous_actions
        ):
    ...
    def _expected_improvement(
        ...
        if policy_gain is not None:
            return max(0.0, float(policy_gain))
    ...
    def _validate_action(
        ...
        if action.startswith("switch_to_"):
            ...
            if target_strategy not in {"dense", "bm25", "hybrid"}:
                return False
            if allow_none_strategy or current_strategy is None:
                return True
            return target_strategy != current_strategy
        ...
        if action.startswith("set_top_k_"):
            ...
            return (
                ...
                and target_top_k in self.SUPPORTED_TOP_K
            )
    ...
    def run(self, context):
        ...
        if learned_candidate is not None:
            ...
            reason = (
                "The failure-conditioned dev policy has sufficient support "
                "for this retrieval state."
            )
        ...
        if not self._validate_action(
            ...
            reason = "The selected action was invalid for the current retrieval state."
            policy_gain = None
